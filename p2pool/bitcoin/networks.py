...
nets = dict(
    ...
    KickAssCoin=math.Object(
        P2P_PREFIX='fbc0b6d0'.decode('hex'),
        P2P_PORT=55884,
        ADDRESS_VERSION=45,
        RPC_PORT=55883,
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'bitcoindaddress' in (yield bitcoind.rpc_help()) 
        )),
        #SUBSIDY_FUNC=lambda height: 50*100000000 >> (height + 1)//840000,
        SUBSIDY_FUNC=lambda height: 1*420000000,
        POW_FUNC=lambda data: pack.IntType(256).unpack(__import__('ltc_scrypt').getPoWHash(data)),
        BLOCK_PERIOD=120, # s
        SYMBOL='KIK',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'Litecoin') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/KickAssCoin/') if platform.system() == 'Darwin' else os.path.expanduser('~/.KickAssCoin'), 'KickAssCoin.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://explorer.KickAssCoin.net/block/',
        ADDRESS_EXPLORER_URL_PREFIX='http://explorer.KickAssCoin.net/address/',
        TX_EXPLORER_URL_PREFIX='http://explorer.KickAssCoin.net/tx/',
        SANE_TARGET_RANGE=(2**256//1000000000 - 1, 2**256//1000 - 1),
        DUMB_SCRYPT_DIFF=2**16,
        DUST_THRESHOLD=0.03e8,
    ),
    KickAssCoin_testnet=math.Object(
        P2P_PREFIX='fcc1b7dc'.decode('hex'),
        P2P_PORT=19333,
        ADDRESS_VERSION=111,
        RPC_PORT=19332,
